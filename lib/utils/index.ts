...
export function getModels(useSubModel = false) {
  let models: LanguageModelV1[] = []
  const ollamaBaseUrl = process.env.OLLAMA_BASE_URL + '/api'
  const ollamaModel = process.env.OLLAMA_MODEL
  const ollamaSubModel = process.env.OLLAMA_SUB_MODEL
  const openaiApiBase = process.env.OPENAI_API_BASE
  ...
  if (ollamaBaseUrl && ollamaModel) {
    const ollama = createOllama({ baseURL: ollamaBaseUrl })
    if (useSubModel && ollamaSubModel) {
      models.push(ollama(ollamaSubModel))
    }
  }
  if (googleApiKey) {
  ...
  if (anthropicApiKey) {
    models.push(anthropic('claude-3-5-sonnet-20240620'))
  }
  ...
  if (azureApiKey && azureResourceName) {
    ...
    models.push(azure.chat(azureDeploymentName))
  }
  ...
  if (groqApiKey && groqApiModel) {
    ...
    models.push(groq.chat(groqApiModel))
  }
